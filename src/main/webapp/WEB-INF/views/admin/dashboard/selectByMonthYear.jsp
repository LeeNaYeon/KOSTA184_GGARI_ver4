...
<script type="text/javascript">
			...
			$.ajax({
				...
				success:function(data) {  
					...
					$.each(data, function(index, item) {
						...
						/* str += "<tr><th><span>" + item.courseStartDate + "</span></th>"+
							"<th><span>" + coursePriceComma + "</span></th></tr>"	 */	
						for(i=0; i<13;i++){
							alert($("#MonthByResultTable tr:gt("+i+") nth-child(1)").substr(0,2))
							if($("#MonthByResultTable tr:gt("+i+") nth-child(1)").substr(0,2) == item.courseStartDate.substr(3,5)){
								$("#MonthByResultTable tr:gt("+i+") nth-child(2)").innerHTML==coursePriceComma;
							}
						}
					});
					
					//table영역에 추가
		/* 			$("#MonthByResultTable tr:gt(0)").remove();
					$("#MonthByResultTable").append(str);
					$("#MonthByResultTable").show();  */
			...
			$.ajax({
				...
				success:function(data) {  
					...
					$.each(data, function(index, item) {
						...
						$('#bannerResultTable tr:last').after("<tr><th><span>" + adsPriceComma + "</span></th></tr>");
	...
	/* 				$("#bannerResultTable tr:gt(0)").remove();
		...
		$("#monthBySearch").on("click",function(){
			//alert("drawChart");
			$.ajax({
    	  		type:"post",//전송방식
				url:"${pageContext.request.contextPath}/admin/dashboard/selectByMonthDashboardGraph", //서버주소
				data : $("#monthBySearchForm").serialize(),//서버에게 보낼 parameter 정보
				dataType:"json", //서버가 front로 보내주는 데이터 타입(text,html,xml,json)
				success:function(data) {  
                  ...
                  <div class="sidebar">
                   <div class="sidebar-widget category-widget">
                	...
                	<div class="col-md-8" style="flex:77%; max-width:77%">
                    	<div><h3  class="MonthYearImage" style="font-size: 20px"><img alt="MonthYearImage" src="${pageContext.request.contextPath}/resources/images/dashboard/MonthYearImage.png">월/년 총매출</h3></div>
                        	<!-- 관리자 멘토신청자 명단 리스트 화면 -->
                            <div style="margin-top: .5em;">
				            <div class="table-responsive" style="text-align: center;">
				            <form id="monthBySearchForm" method="post">
				            	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">				            	
				            	<div style="margin:0 0 0 5px ;float: right; border-radius: 10px;">
				            		<button type="button" class="btn btn-primary button" id="monthBySearch" >검색하기<span class="btn-shape"></span></button>									
				            	</div>
				                ...
				                <table class="table" id="MonthByResultTable">
				                    <tbody>
